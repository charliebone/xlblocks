...
internal sealed class FunctionCallExpression : IColumnExpression
{
    ...
    public FunctionCallExpression(Token<DataFrameExpressionToken> identifier, ValueOption<IColumnExpression> expressionList)
    {
        _functionName = identifier.StringWithoutQuotes;

        if (expressionList.Match(x => x, () => null!) is ExpressionListExpression expressionListExpression)
